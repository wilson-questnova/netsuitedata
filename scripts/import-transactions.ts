...
async function importTransactions() {
  try {
    ...
    // Read CSV file
    const csvPath = path.join(process.cwd(), '..', 'csv_files', 'transactions', 'TransactionsJuly.csv');
    ...
    console.log(`Found ${rows.length} data rows in CSV (excluding header)`);
    ...
    for (let i = 0; i < rows.length; i += batchSize) {
      ...
      if (transactions.length > 0) {
        ...
        console.log(`✅ Saved batch: ${processedCount} transactions processed...`);
